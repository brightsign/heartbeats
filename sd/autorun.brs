...
Sub Main()
    ...
    autorunVersion$ = "6.7.20" ' BA 3.7.0.20
...
Function EnableDebugging() As Object
	...
	if syncSpec.ReadFromFile("current-sync.xml") or syncSpec.ReadFromFile("local-sync.xml") or syncSpec.ReadFromFile("localToBSN-sync.xml") then
						...
						if udpEventName = "<any>"  then
							...
							if targetMediaState$ <> "" then
								...
								if targetState.type$ = "playFile" then
									for each playFileKey in targetState.filesTable
										if not udpEvents.DoesExist(playFileKey) then
											udpEvents.AddReplace(playFileKey, playFileKey)
										endif
									next
								endif
							endif
		...
		for each userVariableXML in userVariablesXML
			...
			if not userVariables.DoesExist(name$) then
				...
				if userVariable.defaultValue$ <> defaultValue$ then
					...
					bsp.UpdateDBVariableDefaultValue(name$, defaultValue$)
				endif
			endif
			...
			if userVariable.systemVariable$ = "serialNumber" then
				...
				if IsString(bsp.activePresentation$) then
					userVariable.SetCurrentValue(bsp.activePresentation$, false)
				else
...
Sub UpdateDBVariableDefaultValue(name$, defaultValue$)

    params = { cv_param: defaultValue$, vn_param: name$, sri_param:  m.currentDBSectionId% }

    m.userVariablesDB.RunBackground("UPDATE Variables SET DefaultValue=:cv_param WHERE VariableName=:vn_param AND SectionReferenceId=:sri_param;", params)
...
Sub ChangeConnectorVolume(multiplier% As Integer, parameters As Object)
	...
	if lcase(connector$) = "analog" then
		...
		m.usbVolume% = ExecuteChangeConnectorVolume("USB", m.usbVolume% + volumeDelta%)
	endif
    ...
    if type(event) = "roControlDown" and stri(event.GetSourceIdentity()) = stri(m.bsp.controlPort.GetIdentity()) then
        ...
        if IsString(event["EventType"]) then
			...
			if event["EventType"] = "BPControlDown" then
				...
				m.bsp.logging.WriteEventLogEntry(m.stateMachine, m.id$, "sendPluginMessage", pluginName$ + pluginMessage$, "0")
...
Sub RestartXModemTimeoutTimer()
	...
	m.sendTimeoutTimer.SetDateTime(newTimeout)
